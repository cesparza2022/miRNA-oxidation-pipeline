...
# Snakemake scripts run from project root, so relative paths should work
...
if (file.exists(data_helpers_path)) {
  ...
  # Try absolute path based on functions path
  functions_dir <- dirname(snakemake@params[["functions"]])
  data_helpers_path_alt <- file.path(functions_dir, "data_loading_helpers.R")
  if (file.exists(data_helpers_path_alt)) {
    source(data_helpers_path_alt, local = TRUE)
    log_info(paste("Data loading helpers loaded from:", data_helpers_path_alt))
  } else {
...
if (nrow(roc_table) == 0 || !"miRNA_name" %in% names(roc_table)) {
  log_warning("No biomarker data available. Creating empty placeholder figure.")
  
  # Create empty placeholder figure
  p_empty <- ggplot() +
    annotate("text", x = 0.5, y = 0.5, label = "No significant biomarkers\nfound for heatmap", 
