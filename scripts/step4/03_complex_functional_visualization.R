...
initialize_logging(log_file, context = "Step 4.3 - Complex Functional Visualization")

log_section("STEP 4: Complex Functional Visualization (Part of Functional Analysis)")
...
target_data <- tryCatch({
  ...
  handle_error(e, context = "Step 4.3 - Loading target data", exit_code = 1, log_file = log_file)
})
...
go_data <- tryCatch({
  ...
  handle_error(e, context = "Step 4.3 - Loading GO data", exit_code = 1, log_file = log_file)
})
...
kegg_data <- tryCatch({
  ...
  handle_error(e, context = "Step 4.3 - Loading KEGG data", exit_code = 1, log_file = log_file)
})
...
target_comp <- tryCatch({
  ...
  handle_error(e, context = "Step 4.3 - Loading target comparison", exit_code = 1, log_file = log_file)
})
...
validate_output_file(output_figure_a, min_size_bytes = 5000, context = "Step 4.3 - Panel A")
...
# Check if als_genes_data is empty
if (nrow(als_genes_data) == 0) {
  log_info("No ALS-relevant genes data found. Creating placeholder figure.")
  # Create empty placeholder figure
  panel_b <- ggplot() +
    annotate("text", x = 0.5, y = 0.5, label = "No ALS-relevant genes found\nin the dataset", 
             size = 6, hjust = 0.5, vjust = 0.5) +
    labs(title = "Impact on ALS-Relevant Genes",
  ...
  top_mirna <- if (nrow(als_summary) > 0) als_summary$miRNA_name[1] else "N/A"
  top_impact <- if (nrow(als_summary) > 0) round(max(als_summary$total_impact, na.rm = TRUE), 2) else 0
...
ggsave(output_figure_b, panel_b, 
       width = fig_width, height = fig_height, dpi = fig_dpi, bg = "white")
validate_output_file(output_figure_b, min_size_bytes = 5000, context = "Step 4.3 - Panel B")
...
# Check if target_comp is empty
if (nrow(target_comp) == 0) {
  log_info("No target comparison data found. Creating placeholder figure.")
  # Create empty placeholder figure
  panel_c <- ggplot() +
    annotate("text", x = 0.5, y = 0.5, label = "No target comparison data found\nin the dataset", 
             size = 6, hjust = 0.5, vjust = 0.5) +
    labs(title = "Target Prediction Comparison: Canonical vs Oxidized miRNAs",
         subtitle = "No data available") +
    theme_void() +
    theme(plot.title = element_text(size = 14, face = "bold", hjust = 0.5),
          plot.subtitle = element_text(size = 11, color = "grey50", hjust = 0.5))
...
ggsave(output_figure_c, panel_c, 
       width = fig_width, height = fig_height, dpi = fig_dpi, bg = "white")
validate_output_file(output_figure_c, min_size_bytes = 5000, context = "Step 4.3 - Panel C")
...
if (nrow(target_data) == 0) {
  log_info("No target data found. Creating placeholder figure.")
  # Create empty placeholder figure
  panel_d <- ggplot() +
    annotate("text", x = 0.5, y = 0.5, label = "No target data found\nin the dataset", 
             size = 6, hjust = 0.5, vjust = 0.5) +
    labs(title = "Position-Specific Functional Impact",
         subtitle = "No data available") +
    theme_void() +
    theme(plot.title = element_text(size = 14, face = "bold", hjust = 0.5),
          plot.subtitle = element_text(size = 11, color = "grey50", hjust = 0.5))
...
ggsave(output_figure_d, panel_d, 
       width = fig_width, height = fig_height, dpi = fig_dpi, bg = "white")
validate_output_file(output_figure_d, min_size_bytes = 5000, context = "Step 4.3 - Panel D")
log_success(paste("Panel D saved:", output_figure_d))

log_success("Step 4.3 completed successfully - All 4 figures generated separately")
