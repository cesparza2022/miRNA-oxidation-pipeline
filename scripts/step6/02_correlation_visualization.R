#!/usr/bin/env Rscript
# ============================================================================
# STEP 6.2: Expression vs Oxidation Correlation Visualization
# ============================================================================
# Purpose: Generate comprehensive figures showing correlation between expression
#          and oxidation patterns
# 
...
#   input: Correlation and expression summary tables
...
initialize_logging(log_file, context = "Step 6.2 - Correlation Visualization")

log_section("STEP 6.2: Expression vs Oxidation Correlation Visualization")
...
panel_a <- ggplot(correlation_data, aes(x = estimated_rpm, y = total_gt_counts)) +
  ...
  labs(
    ...
    subtitle = paste("Correlation between RPM and G>T mutations in seed region (", seed_start, "-", seed_end, ") |",
...
panel_b <- ggplot(expression_comparison, aes(x = expression_category, y = total_gt_counts, fill = expression_category)) +
  ...
  labs(
    ...
    subtitle = paste("G>T mutations in seed region (", seed_start, "-", seed_end, ") grouped by expression level |",
...
log_success("Step 6.2 completed successfully - All 2 figures generated separately")
