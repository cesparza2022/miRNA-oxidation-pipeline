#!/usr/bin/env Rscript
# ============================================================================
# STEP 5: Expression vs Oxidation Correlation Analysis
# ============================================================================
# Purpose: Analyze correlation between miRNA expression levels (RPM) and 
#          oxidative damage (G>T mutations in seed region)
#          This step runs early in the pipeline (after Step 2) to examine
#          relationships before functional interpretation.
#
# Execution order: Step 1 → Step 1.5 → Step 2 → Step 3 → Step 5 (parallel with 4,6)
#
# This step uses SIGNIFICANT G>T mutations in seed region:
# - G>T mutations only (str_detect(pos.mut, ":GT$"))
# - In seed region (positions 2-8)
# - Statistically significant (FDR < alpha)
# - Higher in disease group (log2FC > threshold)
#
# Snakemake parameters:
#   input: Statistical comparisons from Step 2, VAF-filtered data, expression data
...
initialize_logging(log_file, context = "Step 5.1 - Expression-Oxidation Correlation")

log_section("STEP 5: Expression vs Oxidation Correlation Analysis")
...
if (nrow(significant_gt) == 0) {
  log_warning("No significant G>T mutations in seed region found. Skipping Step 5.1.")
  write_csv(tibble(), output_correlation)
  write_csv(tibble(), output_expression_summary)
  log_success("Step 5.1 completed (skipped due to no data).")
...
if (nrow(correlation_data) < 2) {
  ...
  log_warning("Creating empty correlation table with warning message.")
  
  # Create empty correlation summary with warning
  correlation_summary <- tibble(
    method = "Pearson",
    correlation_coefficient = NA_real_,
    p_value = NA_real_,
    n_miRNAs = nrow(correlation_data),
    significance = "Insufficient data (n < 2)",
    note = paste("Only", nrow(correlation_data), "miRNA(s) with both expression and oxidation data")
  )
  ...
  correlation_result <- tryCatch({
    ...
    tryCatch({
      ...
      log_warning(paste("Log-transformed correlation also failed:", e2$message))
      NULL
    })
  })
  ...
  if (!is.null(correlation_result)) {
    correlation_summary <- tibble(
      ...
      note = if (!is.null(correlation_spearman)) {
        paste("Spearman rho =", round(correlation_spearman$estimate, 3), 
              "(p =", round(correlation_spearman$p.value, 4), ")")
      } else {
        "Spearman correlation not available"
      }
    )
    ...
    log_warning("All correlation methods failed. Creating empty summary.")
    correlation_summary <- tibble(
      method = "Pearson",
      correlation_coefficient = NA_real_,
      p_value = NA_real_,
      n_miRNAs = nrow(correlation_data),
      significance = "Correlation failed",
      note = "All correlation methods failed"
    )
  ...
  # Add Spearman correlation to summary if available
  if (!is.null(correlation_spearman) && nrow(correlation_data) >= 2) {
  ...
  # Log correlation results if available
  if (!is.null(correlation_result) && nrow(correlation_data) >= 2) {
  ...
  # Add correlation columns to correlation_table if not already present
  if (!"correlation_coefficient" %in% names(correlation_table)) {
    correlation_table <- correlation_table %>%
      mutate(
        correlation_coefficient = if (!is.null(correlation_result)) as.numeric(correlation_result$estimate) else NA_real_,
        p_value = if (!is.null(correlation_result)) correlation_result$p.value else NA_real_,
        note = if (nrow(correlation_data) < 2) "Insufficient data for correlation" else ""
      )
  }
}

# ============================================================================
# CREATE CORRELATION TABLE (ensure all columns are present)
# ============================================================================

# Ensure correlation_table has all necessary columns
if (!exists("correlation_table") || nrow(correlation_table) == 0) {
  correlation_table <- correlation_data %>%
    select(
      miRNA_name,
      estimated_rpm,
