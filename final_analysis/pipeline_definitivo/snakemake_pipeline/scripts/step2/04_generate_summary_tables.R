...
alpha <- if (exists("config") && !is.null(config$analysis$alpha)) {
...
# Helper to safely get column value with fallback
safe_get <- function(df, col_name, fallback_col = NULL) {
  if (col_name %in% names(df)) {
    return(df[[col_name]])
  } else if (!is.null(fallback_col) && fallback_col %in% names(df)) {
...
significant_mutations <- merged_data %>%
  ...
  select(
    ...
    position,
    mutation_type,
    any_of(c("ALS_mean", names(merged_data)[grepl("ALS_mean|^ALS$", names(merged_data), ignore.case = TRUE)])),
    any_of(c("Control_mean", names(merged_data)[grepl("Control_mean|^Control$", names(merged_data), ignore.case = TRUE)])),
